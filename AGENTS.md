...
계획서: https://Codex.ai/artifact/3x973zhj91rmq7XnszFPbp (범위 결정·대안 비교). 요약:
...
- **번역 훅**: `.husky/pre-commit` → `scripts/translate/pre-commit.mjs`(+ `prompt.md` 문체·마크다운
  ...
  (`git show :경로`)을 `Codex -p --model Codex-opus-5-5 --tools ""`로 통째 번역(매번 전체 재번역),
...
- **함정들(이번에 밟음)**:
  ...
  - nvm 의 `Codex` CLI 2.1.215 는 `Codex-opus-5-5`를 거부했다(2.1.280 이상 필요) → `Codex update`.
    훅은 node·Codex 를 못 찾으면 `~/.nvm/nvm.sh`를 불러온다(GUI 커밋 대비).
  - `Codex -p --bare`는 API 키 전용이라 못 쓴다. 저장소 AGENTS.md 를 번역 맥락에 안 넣으려고
    임시 폴더를 cwd 로 실행한다.
